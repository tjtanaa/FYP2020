...
# compress the video to 1080p
# downsample the video frames and convert the video to 720p
# compress the video with x264 x265 to 720p 1k bitrate 500 bitrate
# ------------------------------------parameters------------------------------#
...
parser.add_argument('--disk_path', default="D:/Github/FYP2020/dataset/4K", help='the path to save the dataset')
...
parser.add_argument('--process', default=1, type=int, help='select process to run')
...
def recompress_videos(codec ='libx264'):
    # type of codec:
    #   * libx264
    #   * libx265

    output_dir = os.path.join(Flags.disk_path, 'HR_' + codec)
    if(not os.path.exists(output_dir)): os.mkdir(output_dir)
    for key in video_data_dict.keys():
        vid_path = os.path.join(Flags.disk_path, '{}.mp4'.format(key))

        # output_path = os.path.join(HD_PATH)

        cmd = ['ffmpeg', '-i', vid_path, 
                '-s', '1280x720',
                '-vcodec', codec,
                '-acodec', 'copy',
                os.path.join(output_dir, '{}.mp4'.format(key))]

        subprocess.call(cmd)

def prepare_DS_video(codec = 'libx264'):
    saveframes = not Flags.TEST
    for keys in video_data_dict:    
            # get training frames
...
if __name__ == '__main__':
    ...
    if(Flags.process == 0):
        download_video()
    if(Flags.process == 1):
        recompress_videos(codec='libx264')
    if(Flags.process == 2):
        recompress_videos(codec='libx265')
...
#     vid_path = os.path.join(_4K_PATH, 'Skiing - 2497.mp4')
#     # output_path = os.path.join(HD_PATH)

#     cmd = ['ffmpeg', '-i', os.path.join(_4K_PATH,'Skiing - 2497.mp4'), 
#             '-s', '1280x720', 
#             '-r', '30', 
#             '-b:v', '4000k', 
#             '-vcodec', 'libx264',
#             '-acodec', 'copy',
#             os.path.join(_HD_PATH, 'output.mp4')]
