...
for epoch in range(st_epoch,Flags.epoch):
    ...
    for m, (X,Y) in enumerate(train_loader):
        ...
        if m % Flags.sample_interval == 0:
            ...
            save_image(gen_imgs.data[:25],  os.path.join(test_dir,"train_%d.png") % m, nrow=5, normalize=True)
            ...
            with torch.set_grad_enabled(False):
                ...
                for k, (X,Y) in enumerate(validation_loader):
                    ...
                    if(k == 0):
                        ...
                        save_image(gen_imgs.data[:25],  os.path.join(test_dir,"val_%d_%d.png") % (epoch, k), nrow=5, normalize=True)
