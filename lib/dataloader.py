...
def video_frames_generator(FLAGS=None):
    ...
    print("input_video_dir: {}".format(FLAGS.input_video_dir))
    ...
    while True:
        ...
        # sequence_starting_index = rand_gen(low=0, high=(num_image_list_LR_t_cur - FLAGS.batch_size), size = FLAGS.batch_size)

        # # suitable for one-shot iterator
        # for i in range(num_image_list_LR_t_cur // FLAGS.batch_size): # number of steps per epoch
        #     for batch in range(FLAGS.batch_size): # building the batch
        #         lr_imgs_sequence = []
        #         hr_imgs_sequence = []
        #         for t in range(FLAGS.RNN_N): # there are five sequence
        #             index = sequence_starting_index[batch]
        #             lr_img_path = image_list_LR_r[t][index]
        #             hr_img_path = image_list_HR_r[t][index]
        #             print("lr_img_path:{}\t hr_img_path:{}".format(lr_img_path,hr_img_path))
        #             lr_img = cv.imread(lr_img_path) # read rgb image
        #             lr_img  = np.divide(lr_img, 255)
        #             lr_imgs_sequence.append(lr_img)
        #             hr_img = cv.imread(hr_img_path) # read rgb image
        #             hr_img  = np.divide(hr_img, 255)
        #             hr_imgs_sequence.append(hr_img)

        #         batch_lr_imgs.append(lr_imgs_sequence)
        #         batch_hr_imgs.append(hr_imgs_sequence)
        #     if len(batch_lr_imgs) == FLAGS.batch_size:
        #         yield (np.asarray(batch_lr_imgs), np.asarray(batch_hr_imgs))
        #         batch_lr_imgs = []
        #         batch_hr_imgs = []

        index = rand_gen(low=0, high=(num_image_list_LR_t_cur - FLAGS.batch_size), size = 1)

        # suitable for batch iterator
        for i in range(num_image_list_LR_t_cur): # number of steps per epoch
            lr_imgs_sequence = []
            hr_imgs_sequence = []
            for t in range(FLAGS.RNN_N): # there are five sequence
                ...
                lr_img_path = image_list_LR_r[t][index[0]]
                hr_img_path = image_list_HR_r[t][index[0]]
                lr_img = cv.imread(lr_img_path) # read rgb image
                lr_img  = np.divide(lr_img, 255) # normalizing image
                lr_imgs_sequence.append(lr_img)
                hr_img = cv.imread(hr_img_path) # read rgb image
                hr_img  = np.divide(hr_img, 255) # normalizing image
                hr_imgs_sequence.append(hr_img)

            yield np.asarray(lr_imgs_sequence), np.asarray(hr_imgs_sequence)



def loadLRHR(FLAGS):
    with tf.device('/cpu:0'):
        dataset = tf.data.Dataset.from_generator(lambda: video_frames_generator(FLAGS), (tf.float32, tf.float32))
        dataset = dataset.shuffle(buffer_size=10)
        dataset = dataset.batch(FLAGS.batch_size)
        dataset = dataset.repeat(count=FLAGS.max_epoch)
        ...
        #     image = lr_data[0][0]
        #     cv.imshow("lr_img", image)

        #     image = hr_data[0][0]
        #     cv.imshow("HR_images",image)
        #     cv.waitKey(0)
...
def frvsr_gpu_data_loader(FLAGS, useValData_ph): # useValData_ph, tf bool placeholder, whether to use validationdata
    ...
    with tf.name_scope('load_frame_cpu'):
        with tf.name_scope('train_data'):
            ...
            train_dataset = loadLRHRfunc(FLAGS)
            # input_batch_list, output_batch_list, num_image_list_LR_t_cur, num_image_list_HR_t_cur = loadLRHRfunc(FLAGS, tar_size)
        ...
        #     vald_input_batch_list, vald_output_batch_list, vald_num_image_list_LR_t_cur, vald_num_image_list_HR_t_cur = loadHRLRfunc(valFLAGS, tar_size)
            val_dataset = loadLRHRfunc(valFLAGS)
    ...
    for lr_images, hr_images in train_dataset.take(-1):
        ...
        # print(tf.transpose(lr_images,[1,0,2,3,4]).get_shape().as_list())
        # cv.imshow("HR_images",tf.transpose(lr_images,[1,0,2,3,4])[0][0].numpy())
        # cv.waitKey(0)

        LR_images = tf.transpose(lr_images,[1,0,2,3,4])
        HR_images = tf.transpose(hr_images,[1,0,2,3,4])

    for lr_images, hr_images in val_dataset.take(-1):
        LR_images_vald = tf.transpose(lr_images,[1,0,2,3,4])
        HR_images_vald = tf.transpose(hr_images,[1,0,2,3,4])
