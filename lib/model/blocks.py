...
class DilatedKronNLBlockND(nn.Module):
    def __init__(self, in_channels, inter_channels=None, mode='embedded', 
        ...
        # define theta and phi for all operations except gaussian
        if self.mode == "embedded" or self.mode == "dot" or self.mode == "concatenate":
            self.theta = conv_nd(in_channels=self.in_channels, out_channels=self.inter_channels, kernel_size=5, padding=2, stride=2)
            self.phi = conv_nd(in_channels=self.in_channels, out_channels=self.inter_channels, kernel_size=5, padding=2, stride=2)
            self.diagonal_matrix = torch.eye(6, dtype=None, requires_grad=True, device=device)
        if self.mode == "concatenate":
            self.W_f = nn.Sequential(
                    nn.Conv2d(in_channels=self.inter_channels * 2, out_channels=1, kernel_size=1),
                    nn.ReLU()
