...
import torch
from .blocks import NLBlockND, DilatedNLBlockND


class TNLRGAN_G(nn.Module):
    def __init__(self, in_channels, out_channels, temporal=False, temporal_sequence=0):
        super(TNLRGAN_G, self).__init__()
        ...
        if self.temporal:
            ...
            self.nl_block = NLBlockND(in_channels=512, mode='embedded', dimension=3, bn_layer=True)
            ...
            self.nl_block = NLBlockND(in_channels=512, mode='embedded', dimension=2, bn_layer=True)
