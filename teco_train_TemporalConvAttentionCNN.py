...
from lib.model import ConvAttentionCNN, TemporalConvAttentionCNN
...
parser.add_argument('--model', default="TemporalConvAttentionCNN", type=str, help='the path to save the dataset')
...
elif Flags.model == 'TemporalConvAttentionCNN':
    model = TemporalConvAttentionCNN(C,C, Flags.tseq_length).to(device)

optimizer = optim.Adam([
    {'params': model.conv1.parameters()},
    {'params': model.conv_attention2.parameters()},
    {'params': model.conv2.parameters()},
    {'params': model.conv3.parameters()},
    {'params': model.conv4.parameters(), 'lr': Flags.lr * 0.1},
], lr=Flags.lr)
...
train_loader_params = {'batch_size': 4, 'num_workers': 4,'sampler': train_sampler}
        ...
        if m % 1000 == 0:
            with torch.set_grad_enabled(False):
                ...
                logger.info("[Epoch %d/%d] [tRec loss: %.5e][vRec loss: %.5e]" 
                    % (epoch, Flags.epoch, running_rec_loss/ (m + 1), val_rec_loss))
